...
class UpdateItem extends Component<Props, State> {
    constructor(props: any) {
        ...
        LocalShooppe.map((item: any) => {
            console.log(item)
            this.props.propsProductID == item.idProduct ? this.state = { ...item } : console.log(item.nameProduct)
        })
    }
    render() {
        return (
            ...
            <div className='mark' style={{}} >
                <div className="content-updateitem">
                    ...
                    <div className="item">
                        ...
                        <input type="text" id="nameProduct" name="name" defaultValue={this.state.nameProduct} onChange={(event) => [
                            this.setState({ nameProduct: event.target.value })
                        ]} />
                    </div>
                    <div className="item">
                        <label htmlFor="afterSale">Giá bán</label>
                        <input type="number" id="afterSale" name="afterSale" defaultValue={this.state.afterSale} onChange={(event) => [
                            this.setState({ afterSale: event.target.valueAsNumber })
                        ]} />
                    </div>
                    <div className="item">
                        <label htmlFor="beforSale">Giá gốc</label>
                        <input type="number" id="beforSale" name="beforSale" defaultValue={this.state.beforSale} onChange={(event) => [
                            this.setState({ beforSale: event.target.valueAsNumber })
                        ]} />
                    </div>
                    <div className="item">
                        <label htmlFor="imgProduct">Hình ảnh sản phẩm</label>
                        <input type="url" id="imgProduct" name="imgProduct" defaultValue={this.state.imgProduct} onChange={(event) => [
